...
use super::{
    bezier_surface::BezierSurfaceBuilder,
    builder::{IParametricBuilder, ITriangulatedBuilder},
    TriangulatedMesh,
};

type Bezier16IndicesArray = [u32; 16];

static INDICES: [Bezier16IndicesArray; 32] = [
...
static VERTICES: [(f64, f64, f64); 306] = [
    (1.4, 0.0, 2.4),
    (1.4, -0.784, 2.4),
    (0.784, -1.4, 2.4),
    (0.0, -1.4, 2.4),
...
impl ITriangulatedBuilder for TeapotBuilder {
    fn triangulated(self) -> super::TriangulatedMesh {
        // 各部位ごとにベジエ曲面を生成し、つなぎあわせてティーポットを構成します。
        // TODO: ベジエ曲面上で制御点と一致する頂点はだぶりがあるので、それを解消する

        let mut index_stride: usize = 0;
        ...
        for bezier_indices in INDICES {
            let bezier_surface = BezierSurfaceBuilder::new_with_div(self.div, self.sub_div)
                .params(|mut x| {
                    for i in 0..bezier_indices.len() {
                        ...
                        x[i].0 = vertex.0 as f32;
                        x[i].1 = vertex.1 as f32;
                        x[i].2 = vertex.2 as f32;
                    }
            ...
            for triangle in bezier_surface.triangles() {
                for index in [triangle.0, triangle.1, triangle.2] {
                    let vertex = bezier_surface.vertex(index);
                    vertices.push(vertex.0);
                    vertices.push(vertex.1);
                    vertices.push(vertex.2);
                }

                indices.push(index_stride + 0);
                indices.push(index_stride + 1);
                indices.push(index_stride + 2);
                index_stride += 3;
            }
        }
